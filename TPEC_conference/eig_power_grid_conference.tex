...
We seek informative probes within a budget of $T$ design steps.
Here $k=1,\ldots,T$ indexes decisions, $t$ is elapsed
physical time since the start of the current probe window, and $\tau_k$
is the selected probe duration.
...
\label{eq:swing}
\end{equation}
Dots denote derivatives with respect to physical time $t$. The damping $D_i$ and network
...
To obtain information about these parameters, each experiment applies a
raised-cosine power pulse at bus 1 with fixed
amplitude $A=0.05$ pu and selected duration $\tau_k$:
\begin{equation}
v_{1,k}(t)=
\begin{cases}
\dfrac{A}{2}\bigl[1-\cos(2\pi t/\tau_k)\bigr],&0\le t\le\tau_k,\\
0,&\tau_k<t\le T_{\mathrm{window}}.
\end{cases}
\label{eq:probe}
\end{equation}
All other probing inputs are zero. Each observation window lasts
$T_{\mathrm{window}}=3.5$ s. The full angle and speed state at its end becomes the initial
state of the next window, without resetting the system. Consequently,
the response to probe $k$ depends on the complete duration sequence
$\tau_{1:k}=(\tau_1,\ldots,\tau_k)$.

Write $\Delta f_{1,k}(t;\theta,\tau_{1:k})$ for the resulting bus-1
frequency deviation. The noise-free observation is the signed endpoint
rate of change of frequency (RoCoF), denoted by $\mu_k$:
\begin{equation}
\mu_k(\theta,\tau_{1:k})=
\frac{\Delta f_{1,k}(T_{\mathrm{window}})-\Delta f_{1,k}(T_{\mathrm{window}}-\Delta t_{\mathrm{samp}})}{\Delta t_{\mathrm{samp}}}.
\label{eq:sensor}
\end{equation}
The frequency arguments $\theta,\tau_{1:k}$ are suppressed in the
numerator. With sampling interval $\Delta t_{\mathrm{samp}}=0.025$ s, the measured
response follows
\begin{equation}
y_k=\mu_k(\theta,\tau_{1:k})+\epsilon_k,
...
subsequent decisions. The data after step $k$ are the ordered design--observation
history $h_k=((\tau_j,y_j))_{j=1}^{k}$, with $h_0=\varnothing$.
A policy $\pi$ selects each duration using only $h_{k-1}$; it has no
access to the true parameters or physical state. Feasible durations satisfy
\begin{equation}
\begin{aligned}
0.2&\le\tau_k\le3, &&k=1,\ldots,T,\\
\tau_k-\tau_{k-1}&\ge d_{\min}, &&k=2,\ldots,T,
\end{aligned}
\label{eq:bounds}
\end{equation}
where $d_{\min}=0.01$ s. At step $k$, the upper limit is tightened to
$3-(T-k)d_{\min}$ to leave room for the remaining probes; the lower
limit is $0.2$ initially and $\tau_{k-1}+d_{\min}$ thereafter.
...
$p(y_k\mid\theta,h_{k-1},\tau_k)$ with mean
$\mu_k(\theta,\tau_{1:k})$ and variance $\sigma^2$.
Bayes' rule updates parameter uncertainty after each measurement:
\begin{equation}
p(\theta\mid h_k)=
\frac{p(y_k\mid\theta,h_{k-1},\tau_k)\,p(\theta\mid h_{k-1})}
{p(y_k\mid h_{k-1},\tau_k)}.
\label{eq:posterior}
\end{equation}
...
\begin{equation}
\mathcal L_k(\theta)=\prod_{j=1}^{k}
p(y_j\mid\theta,h_{j-1},\tau_j).
\label{eq:likelihood}
\end{equation}
The sampled information score is then
\begin{equation}
\widehat I_k=\log\frac{\mathcal L_k(\theta^{(0)})}
{\frac{1}{L+1}\sum_{\ell=0}^{L}\mathcal L_k(\theta^{(\ell)})}.
\label{eq:spce}
\end{equation}
...
methods below implement these alternatives using the same feasible
...
Random draws $u_k\sim\mathcal U(0,1)$ independently at each step and
sets $\tau_k=\tau_k^{\min}+(\tau_k^{\max}-\tau_k^{\min})u_k$.
Here $\tau_1^{\min}=0.2$, $\tau_k^{\min}=\tau_{k-1}+d_{\min}$ for $k>1$,
and $\tau_k^{\max}=3-(T-k)d_{\min}$. Thus sampling is uniform within
...
$R_k=\widehat I_k-\widehat I_{k-1}$, with $\widehat I_0=0$ and
...
\node[box,fill=blue!5] (select) at (0,0)
 {Choose a feasible duration $\tau_k$\\Selected design method\\Start at $k=1$, $h_0=\varnothing$};
...
\node[box,fill=blue!5] (observe) at (11.6,0)
 {Measure the response\\Observe noisy endpoint RoCoF $y_k$\\Response depends on $\theta$};
\node[box,fill=gray!5] (history) at (11.6,-2.3)
 {Append design and observation\\$h_k=(h_{k-1},(\tau_k,y_k))$\\History retains all preceding probes};
\node[box,fill=green!5,minimum height=1.5cm] (posterior) at (5.8,-2.3)
 {Update parameter uncertainty\\$p(\theta\mid h_k)\propto$\\
 $p(y_k\mid\theta,h_{k-1},\tau_k)$\\$\times\,p(\theta\mid h_{k-1})$\\Initial belief: $p_0(\theta)$};
...
\draw[flow] (history.south) -- (11.6,-3.65)
 -- node[note,pos=.5] {Observed history $h_k$} (0,-3.65) -- (next.south);
\draw[flow] (next.north) -- node[note,left] {$k\leftarrow k+1$} (select.south);
\node[note,anchor=north] at (5.8,-4.05)
 {Repeat while $k<T$. Aim: greater expected reduction in uncertainty about $M$ and $K$ at the same probe budget.};
\end{tikzpicture}
...
\section{Conclusion}
Sequential Bayesian design offers a way to select more informative
power-system probes within a fixed measurement budget. Under the specified
IEEE9 and IEEE14 models, DAD, RL-sBOED and Step-DAD achieve higher mean
sPCE than Random, Fixed and Myopic at three to five design steps.
DAD and RL-sBOED combine these gains with low online computation by
learning reusable policies offline. Four-update Step-DAD gives modest
mean gains over DAD on IEEE9, but no consistent improvement on IEEE14,
while adding substantial online cost.

These findings support offline-trained adaptive probing within the tested
settings. Near-ceiling IEEE14 scores prevent strong rankings within the
