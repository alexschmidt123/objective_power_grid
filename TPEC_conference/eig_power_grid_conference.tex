...
subsections specify how each decision is obtained; Algorithm~\ref{alg:probing}
in the experimental settings summarizes the complete study workflow. These are application-specific adaptations
...
Algorithm~\ref{alg:probing} organizes the experiment into offline
preparation, sequential probing, and final scoring. Method-specific
particle updates and policy refinement follow Section~III.

\begin{figure}[t]
\small
\refstepcounter{algorithm}\label{alg:probing}
\hrule\smallskip
\textbf{Algorithm \thealgorithm: Power-system probing workflow}
\smallskip\hrule\smallskip
\begin{algorithmic}[1]
\REQUIRE Grid model, parameter prior, probe constraints, sensor model and design-step budget $T$
\STATE Fix a common simulation and evaluation protocol for all six methods.
\STATE Train offline designs/policies where needed; select checkpoints on validation cases and record training time.
\STATE Generate evaluation systems and measurement-noise draws paired across methods; keep true parameters hidden from design methods.
\FOR{each method and evaluation system}
\STATE Initialize the grid at equilibrium, set $h_0=\varnothing$, and initialize method-specific planning state.
\FOR{$t=1,\ldots,T$}
\STATE Start the online decision timer.
\STATE Use $h_{t-1}$ to update the planner or refine the policy if required by the selected method.
\STATE Select a feasible probe duration $\tau_t\in[a_t,b_t]$; stop the decision timer.
\STATE Apply the fixed-amplitude pulse at bus 1 and simulate the complete recording window.
\STATE Collect the noisy endpoint-RoCoF observation $y_t$; append $(\tau_t,y_t)$ to the history.
\STATE Carry the terminal grid state into the next design step without resetting.
\ENDFOR
\STATE Compute terminal sPCE separately; sum decision times over the sequence.
\ENDFOR
\STATE Aggregate information scores and computation times using the declared training/evaluation-seed groups.
\ENSURE Information gained at equal probe budgets, offline training cost, and online decision cost
\end{algorithmic}
\smallskip\hrule
\end{figure}
