...
such as inertia and effective frequency response. Active probing can reveal
these parameters, but the information obtained depends on which inputs
are applied. This paper investigates sequential Bayesian optimal experimental
design to acquire more information with the same number of probes.
We formulate probe-duration selection as an expected-information-gain
maximization problem, using previous observations to guide subsequent
probes while accounting for the evolving physical state. We compare six
existing methods on reduced IEEE9 and IEEE14 models with three, four and
five design steps: deep adaptive design (DAD), reinforcement-learning-based
sequential design (RL-sBOED), Step-DAD, Myopic, Fixed and Random.
Performance is assessed through a contrastive lower bound on information
gain, seed variability, and offline and online computation.
On both networks, DAD, RL-sBOED and Step-DAD achieve higher mean information
scores than the three baselines at every tested design-step count.
At three design steps, DAD exceeds Myopic by
0.163 nats on IEEE9 and 0.467 nats on IEEE14. Its mean online computation
per sequence is 0.54 and 1.22 ms, respectively, compared with 9.31 and
10.00 s for Myopic. Step-DAD provides modest additional mean gains over
DAD on IEEE9 but no consistent gain on IEEE14, with substantially greater
online cost. IEEE14 scores approach the estimator ceiling, limiting
comparisons among the strongest methods. These results support offline-trained
sequential design as an effective approach to informative probing with
low online computation under the studied conditions.
\end{abstract}
\begin{IEEEkeywords}
