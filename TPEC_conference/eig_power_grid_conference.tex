...
Here $t=1,\ldots,T$ indexes decisions, $t_{\mathrm{loc}}$ is elapsed
physical time since the start of the current probe window, and $\tau_t$
...
\label{eq:swing}
\end{equation}
Dots denote derivatives with respect to physical time $t_{\mathrm{loc}}$. The damping $D_i$ and network
...
\begin{equation}
v_{1,t}(t_{\mathrm{loc}})=
\begin{cases}
\dfrac{A}{2}\bigl[1-\cos(2\pi t_{\mathrm{loc}}/\tau_t)\bigr],&0\le t_{\mathrm{loc}}\le\tau_t,\\
0,&\tau_t<t_{\mathrm{loc}}\le W.
\end{cases}
\label{eq:probe}
\end{equation}
...
Write $\Delta f_{1,t}(t_{\mathrm{loc}};\theta,\tau_{1:t})$ for the resulting bus-1
frequency deviation. The noise-free observation is the signed endpoint
rate of change of frequency (RoCoF), denoted by $\mu_t$:
\begin{equation}
\mu_t(\theta,\tau_{1:t})=
\frac{\Delta f_{1,t}(W)-\Delta f_{1,t}(W-\Delta t_{\mathrm{samp}})}{\Delta t_{\mathrm{samp}}}.
\label{eq:sensor}
\end{equation}
