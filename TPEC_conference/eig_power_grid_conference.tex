...
The probe design is $\xi_t=(\mathrm{amp}_t,b_t,d_t)$: amplitude,
injection bus and pulse duration. This study fixes $\mathrm{amp}_t=0.05$ pu
and $b_t=1$; only $d_t$ is optimized. The raised-cosine input for
$0\le t_{\mathrm{probe}}\le d_t$ is
\begin{equation}
v_{b_t,t}(t_{\mathrm{probe}})
 =\frac{\mathrm{amp}_t}{2}\bigl[1-\cos(2\pi t_{\mathrm{probe}}/d_t)\bigr].
\label{eq:probe}
\end{equation}
The input is zero for $d_t<t_{\mathrm{probe}}\le T_{\mathrm{probe}}$
...
Write $\Delta f_{1,t}(t_{\mathrm{probe}};\theta,\xi_{1:t})$ for the resulting bus-1
...
response follows
\begin{equation}
y_t=\mu_t(\theta,\xi_{1:t})+\epsilon_t,
...
history $h_t=((\xi_k,y_k))_{k=1}^{t}$, with $h_0=\varnothing$.
...
$p(y_t\mid\theta,h_{t-1},\xi_t)$ with mean
$\mu_t(\theta,\xi_{1:t})$ and variance $\sigma^2$.
Bayes' rule updates parameter uncertainty after each measurement:
\begin{equation}
p(\theta\mid h_t)=
\frac{p(y_t\mid\theta,h_{t-1},\xi_t)\,p(\theta\mid h_{t-1})}
{p(y_t\mid h_{t-1},\xi_t)}.
\label{eq:posterior}
\end{equation}
...
sets $d_t=d_t^{\min}+(d_t^{\max}-d_t^{\min})u_t$.
...
\node[box,fill=blue!5] (select) at (0,0)
 {Choose design $\xi_t$ (optimize $d_t$)\\Selected design method\\Start at $t=1$, $h_0=\varnothing$};
...
\node[box,fill=gray!5] (history) at (11.6,-2.3)
 {Append design and observation\\$h_t=(h_{t-1},(\xi_t,y_t))$\\History retains all preceding probes};
\node[box,fill=green!5,minimum height=1.5cm] (posterior) at (5.8,-2.3)
 {Update parameter uncertainty\\$p(\theta\mid h_t)\propto$\\
 $p(y_t\mid\theta,h_{t-1},\xi_t)$\\$\times\,p(\theta\mid h_{t-1})$\\Initial belief: $p_0(\theta)$};
