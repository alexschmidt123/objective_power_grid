...
\maketitle
\begin{abstract}
Uncertainty in inertia and frequency response limits how accurately a
power-system model can predict the response to disturbances. Controlled
probing can reduce this uncertainty, but the information obtained depends
on which probing signals are applied. Choosing each signal through online
optimization also incurs computational cost. This paper investigates how
to select informative sequences of probes while accounting for the
computation required to choose them. We formulate probe-duration selection
as a sequential Bayesian experimental-design problem that maximizes
expected information gain about uncertain system parameters. Each decision
can use previous noisy frequency observations, and the physical state
evolves continuously between probes. We apply six existing approaches:
deep adaptive design, reinforcement-learning-based sequential design,
Step-DAD online policy refinement, myopic optimization, fixed design, and
random design. The comparison uses reduced IEEE9 and IEEE14 models with
common probing constraints and evaluation conditions. Information is
assessed through a contrastive lower bound, while offline training and
online decision times are reported separately. The study examines whether
observation-dependent and full-sequence designs acquire more information
than simpler baselines, and whether online refinement provides sufficient
additional information to justify its computational cost.
\end{abstract}
\begin{IEEEkeywords}
