...
\textit{[IEEE9 results and the final cross-system conclusion pending.]}
\end{abstract}
\begin{IEEEkeywords}
...
\label{sec:results}
\noindent\textit{Draft status: IEEE14 results are measured; IEEE9 table
zeros and empty plot panels remain placeholders. Both campaigns currently
use three evaluation seeds; the five-seed target is incomplete.}

\subsection{Information Versus Number of Design Steps}
\begin{table}[!ht]
\caption{IEEE9 terminal sPCE (nats). Trained methods: mean $\pm$ sample
SD across three training seeds. Random/Myopic: mean $\pm$ sample SD
across evaluation-seed means (five planned; three currently complete).
$L=128$; ceiling 4.8598.
All zeros are placeholders, not results.}
\label{tab:ieee9}
\centering\small
\begin{tabular}{lccc}
\toprule Method & $T=3$ & $T=4$ & $T=5$\\\midrule
Random & $0.00\pm0.00$ & $0.00\pm0.00$ & $0.00\pm0.00$\\
Fixed & $0.00\pm0.00$ & $0.00\pm0.00$ & $0.00\pm0.00$\\
Myopic & $0.00\pm0.00$ & $0.00\pm0.00$ & $0.00\pm0.00$\\
...
\caption{Information versus online computation. IEEE9 remains a
placeholder. IEEE14 points connect $T=3,4,5$ for each method on
Grace A100; vertical bars show the SD defined in Table~\ref{tab:ieee14}.}
\label{fig:tradeoff}
\end{figure}
...
\caption{Validation histories. IEEE9 remains a placeholder.
IEEE14 $T=3$ curves show mean $\pm$ sample SD across three training
seeds at each checkpoint. Step-DAD reuses the trained DAD policy.}
\label{fig:convergence}
\end{figure}

Figure~\ref{fig:convergence} reports the saved validation histories
for IEEE14 at $T=3$. Policies are evaluated using their best validation
checkpoint, rather than necessarily the last update. DAD begins from
a validation-selected initialization, which can reuse the separately
trained Fixed design. The plotted DAD update axis therefore omits
that preceding cost; Table~\ref{tab:offline} reports it separately.

For training seed 101, the selected DAD and RL-sBOED checkpoints
...
evaluation target limit the conclusions; the final IEEE9 comparison
