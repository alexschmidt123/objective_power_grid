...
\maketitle
\begin{abstract}
Understanding power-system dynamics requires knowledge of latent inertia
and frequency-response parameters. Active probing can reveal these parameters,
but selecting inputs requires balancing information acquisition against
online computation. We investigate
this trade-off using sequential Bayesian optimal experimental design,
choosing probe durations from previous observations while accounting for
the evolving grid state. On IEEE9 and IEEE14 reduced models, we compare
...
methods achieve higher mean contrastive information scores than the three
...
Inertia and effective frequency response determine how a power system
reacts to disturbances. Understanding these dynamics requires information
about the underlying parameters. Controlled probing can estimate them
from measured frequency responses~\cite{peng2024probing}, but a probe is
useful only insofar as its response distinguishes plausible parameter
values. With a limited experiment budget, choosing informative inputs
is therefore essential.

We study the design question preceding parameter estimation: how should
successive probes be selected to learn more about these parameters with
the same number of measurements? A fixed sequence cannot use newly
observed responses. Myopic Bayesian design adapts to them but optimizes
only the next experiment~\cite{huan2016sequential}. Earlier probes also
change the physical state from which later grid responses develop.
Sequential design must account for both accumulated information and
...
Their availability leaves an application question: under matched grid
probing constraints, does planning over the full sequence improve
information acquisition, and does online refinement justify its cost?

We address this question by formulating continuous probe-duration
selection with a shared sensor model and physical state carried between
experiments. Six existing methods are compared on reduced IEEE9 and
IEEE14 systems at equal probe budgets. We report contrastive information
scores, seed variability, and offline and online computation. Our
...
noise SD 1.0, and a precomputed simulation bank. Its 1,024-particle
posterior entropy reduction has ceiling $\log(1024)=6.9315$ nats and
differs from grid sPCE.

Table~\ref{tab:sir} reports mean $\pm$ sample SD in nats.
...
\par\medskip\noindent\begin{minipage}{\columnwidth}\makeatletter\def\@captype{table}\makeatother\caption{SIR-ODE posterior entropy reduction}\label{tab:sir}\papertablestyle
\begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}}lccc@{}}\toprule Method & $T=3$ & $T=4$ & $T=5$\\\midrule
...
The learned methods have higher mean scores than the baselines, but
near-ceiling values limit distinctions within that group. This benchmark
is not a matched replication of the grid experiments.
