...
Here $t=1,\ldots,T$ indexes design steps, $t_{\mathrm{probe}}$ denotes
elapsed time within each probe window, and $\tau_t$ is the selected
pulse duration.
...
\label{eq:swing}
\end{equation}
Dots denote derivatives with respect to physical time $t_{\mathrm{probe}}$. The damping $D_i$ and network
...
amplitude $A=0.05$ pu and selected pulse duration $\tau_t$. For
$0\le t_{\mathrm{probe}}\le\tau_t$,
\begin{equation}
v_{1,t}(t_{\mathrm{probe}})
 =\frac{A}{2}\bigl[1-\cos(2\pi t_{\mathrm{probe}}/\tau_t)\bigr].
\label{eq:probe}
\end{equation}
The input is zero for $\tau_t<t_{\mathrm{probe}}\le T_{\mathrm{probe}}$
and at all other buses. The observation window is $T_{\mathrm{probe}}=3.5$ s. The full angle and speed state at its end becomes the initial
...
Write $\Delta f_{1,t}(t_{\mathrm{probe}};\theta,\tau_{1:t})$ for the resulting bus-1
frequency deviation. The noise-free observation is the signed endpoint
rate of change of frequency (RoCoF), denoted by $\mu_t$:
\begin{equation}
\mu_t(\theta,\tau_{1:t})=
\frac{\Delta f_{1,t}(T_{\mathrm{probe}})-\Delta f_{1,t}(T_{\mathrm{probe}}-\Delta t_{\mathrm{samp}})}{\Delta t_{\mathrm{samp}}}.
\label{eq:sensor}
\end{equation}
...
Step-DAD achieve higher mean sPCE than Random, Fixed and Myopic at
