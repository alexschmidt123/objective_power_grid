...
\begin{abstract}
This paper formulates sequential Bayesian experiment design for a reduced
power-grid model with uncertain inertia and frequency response. Probe duration
is continuous while injection location and amplitude are fixed. Physical states
carry between recording windows, making the order of probes relevant.
Noisy frequency measurements update a posterior model that determines a
continuous terminal power injection under declared frequency-security criteria.
We distinguish expected information gain (EIG), expected selected minimum safe
control (MSC), and finite-loss mean objective cost of uncertainty (MOCU).
Posterior coverage is a model-based decision preference; physical safety is
assessed separately on held-out true systems. Six design methods share the
physics and objective-specific terminal rule, with their numerical adaptations
stated explicitly. IEEE9 is the implemented non-reset benchmark; the remaining
system extensions and publication-level comparisons require further validation.
\todo{Add quantitative findings only from completed matched non-reset runs.}
\end{abstract}
\begin{IEEEkeywords}
Sequential Bayesian experimental design, power-system dynamics, non-reset
probing, minimum safe control, mean objective cost of uncertainty.
\end{IEEEkeywords}
\section{Introduction}
\IEEEPARstart{U}{ncertainty} in inertia and effective frequency response affects
both the interpretation of a probe signal and the control needed for a specified
disturbance. Probing supplies observations; the final engineering decision is
a supplementary power-injection magnitude. These two decisions belong to
different action spaces and are connected through a posterior dynamical model.

We study an ordered sequence of fixed-amplitude, fixed-location probes whose
durations are continuous. Each probe begins when the preceding recording window
ends, retaining its physical state. The objectives are parameter-oriented EIG,
expected selected minimum safe control (MSC), and finite-loss MOCU. Their
comparison asks whether different information requirements lead to different
probe policies and subsequent controls, without requiring any method family
to win by construction.

\section{Related Work}
Controlled excitation motivates the inertia/response identification model
of Peng et al.~\cite{peng2024probing}. Our multi-machine non-reset benchmark
extends that motivation; its sensing, prior and control assumptions are stated
separately rather than attributed to that paper. DAD, RL-sBOED and Step-DAD
provide policy-based sequential design methods~\cite{foster2021dad,blau2022rlsboed,hedman2025stepdad}.
Yoon's MOCU framework and its generalizations evaluate uncertainty through a
specified downstream loss~\cite{yoon2013mocu,boluki2018generalized}.
The present MSC criterion and particular MOCU shortfall loss are declared
application choices. They are not asserted to be engineering standards or
objectives already established by those design-method papers.
...
RL-sBOED measures policy entropy in normalized tanh coordinates $[-1,1]$
and uses automatic temperature tuning with target entropy $-1$ for its
one-dimensional action. This training regularizer is distinct from the
physical noise and terminal-control coverage. Reported evaluation scores
exclude the entropy bonus.
