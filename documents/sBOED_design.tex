...
\title{Sequential Bayesian Experimental Design for Non-Reset Power-Grid Identification and Control}
\author{}
\date{}
\begin{document}
\maketitle
...
physical safety assessment and statistical comparison. The resulting methods
section defines reproducible comparisons without presupposing an empirical
...
The methodological contributions are a common non-reset formulation for these
three objectives, an implementation of six design strategies under identical
physical and observation constraints, and a verification protocol that
separates numerical correctness from optimization and statistical evidence.
The six strategies cover random probing, an optimized fixed sequence, myopic
posterior-based selection, fully amortized DAD and RL-sBOED policies, and
within-episode Step-DAD refinement. The comparison is intended to measure
when horizon planning and adaptation help, without imposing a desired ranking.
The implemented grid benchmark is IEEE9. This paper specifies the methodology;
quantitative conclusions require completed experiments and the validation
described in Section~\ref{sec:experimental-protocol}.

\subsection{Related Work}
\label{sec:related-work}
\subsubsection{Probing for Dynamic Parameter Identification}
...
\subsubsection{Amortized and Semi-Amortized Bayesian Design}
...
\subsubsection{Objective-Based Uncertainty and Experimental Design}
...
The preceding approaches address complementary issues: excitation for
identification, policy amortization, online refinement, and uncertainty
measured through a decision loss. We combine these ideas in one explicitly
specified benchmark rather than claiming a new general-purpose design
algorithm. Random, Fixed and Myopic provide controls for the effects of
offline optimization, observation dependence and lookahead. The common action
constraints, matched evaluation systems and method-specific numerical checks
make differences interpretable within the stated model. They do not establish
optimality, field safety, or superiority beyond that model.

\subsection{Organization of the Paper}
Section~\ref{sec:ieee-system} introduces the IEEE network and reduction.
Section~\ref{sec:dynamics} defines the swing dynamics and latent parameters,
and Section~\ref{sec:probe-design} specifies probing, observations and inference.
Section~\ref{sec:objectives} develops the design objectives.
Sections~\ref{sec:methodology} and~\ref{sec:experimental-protocol} present the
algorithms and experimental protocol, respectively.
Section~\ref{sec:results} is reserved for verified numerical findings, and
Section~\ref{sec:conclusion} closes the paper.

\section{IEEE Test System and Network Representation}
\label{sec:ieee-system}
\subsection{Physical Network and Retained Machines}
...
\subsection{Network Reduction and Injection Mapping}
...
\section{Swing Dynamics and Latent Parameter Space}
\label{sec:dynamics}
\subsection{Inertia and Effective Frequency Response}
...
\subsection{Coupled Machine Dynamics}
...
\subsection{Latent Parameters and Prior Distribution}
...
\section{Probe Experiments and Sequential Design Space}
\label{sec:probe-design}
\label{sec:problem}
\subsection{Probe Waveform and Non-Reset State Evolution}
...
\subsection{Ordered Design Space}
...
\subsection{Maximum-RoCoF Observation Model}
...
\subsection{From Parameter Learning to a Terminal Decision}
...
the increasing feasible interval defined in Section~\ref{sec:problem}.
...
\begin{table}[t]
\caption{Capabilities of the implemented design strategies. Horizon awareness
here means the optimized utility, not merely reserving feasible action space.}
\centering
\begin{tabular}{lccc}
\toprule
Method & \shortstack{Uses\\observations} & \shortstack{Full-horizon\\utility} & \shortstack{Online policy\\refinement}\\
\midrule
Random & No & No & No\\
Fixed & No & Yes & No\\
Myopic & Yes & No & No\\
...
used, stochastic policy actions. Each objective is trained separately.
...
No checkpoint from a previous experiment is loaded.
...
enter a planner or terminal controller by privileged access. Every independent
run trains fresh policies; within-run Step-DAD initialization from its matching
DAD policy is part of the method definition.
...
The specified full-budget EIG comparison uses 2,000 trajectory-batch updates,
...
computational settings, not established convergence thresholds; control-objective
studies must declare their own perturbation and solver budgets.
...
train seeds 101/202/303 and evaluate seeds 1001--1005. This specifies a study
design and does not assert completed experiments.
...
This work formulates sequential Bayesian design for a power-grid experiment
whose physical state persists across probes. The network representation,
swing dynamics, latent parameter space and observation model define a common
basis for comparing information-oriented and control-oriented objectives.
The methodology specifies six design strategies together with the numerical
approximations needed for online inference, policy optimization and terminal
control. In particular, the maximum-RoCoF observation requires a consistent
active-branch derivative when pathwise optimization is used.

The formulation distinguishes parameter information, selected minimum safe
control and the objective cost of uncertainty, while retaining separate
physical safety assessment. Its empirical implications must be established
by completed comparisons, convergence diagnostics and sensitivity analyses.
No numerical ranking or field-level safety guarantee follows from the
formulation alone. Broader validation requires additional independent seeds,
operating conditions and dynamic models beyond the implemented IEEE9 setting.
% Add a concise quantitative conclusion only after Section VIII is populated.
