#!/usr/bin/env python3
"""Search the 281-duration IEEE9 bank for six-duration MOCU spaces.
...
def install_safety_aware_mocu_terminal(*, undercontrol_penalty: float,
    ...
    """Make the structural planner use the exact train/eval Yoon MOCU."""
    def terminal_loss(U, log_w, *, alpha, margin, grid):
        w = normalize_log_weights(log_w)
        # Primary MOCU protocol: hard-safety Yoon IBR action and regret.
        # The penalty arguments remain in the compatibility signature because
        # the structural-audit helper passes them, but are intentionally unused.
        u = float(ibr_max_u_ctrl(np.asarray(U, dtype=np.float64), w))
        return float(belief_mocu(np.asarray(U, dtype=np.float64), w, u))
...
def main():
    ...
    run = {"system":"ieee9", "study_type":"MOCU_full_281_duration_global_local_audit_v3", "objective":"Yoon IBR MOCU identical to training/evaluation", "duration_grid_s":[.20,3.00,.01], "n_duration_options":281, "combination_space":math.comb(281,6), "choose":6, "global_samples":args.global_samples, "global_exact":args.global_exact, "local_parents":args.local_parents, "local_exact":args.local_exact, "finalists":args.finalists, "audit_seeds":list(seeds), "N_obs":args.n_obs, "noise_sigma":args.noise_sigma, "support_size":len(U), "status":"running"}
