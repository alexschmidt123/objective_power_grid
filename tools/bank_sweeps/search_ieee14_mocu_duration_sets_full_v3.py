...
def exact_audit(key, *, durations, centres, U, n_buses, sigma, seeds, outer, inner,
    ...
    passed = (ad_mean >= min_adaptive_advantage and ad_lcb > 0 and
              ...
              min_branches >= min_duration_branches and
              max_dom <= max_dominance and
              (differs or not require_first_duration_differs))
    return {
...
def main():
    ...
    p.add_argument("--min-adaptive-advantage", type=float, default=0.0)
    ...
    def score(keys, label):
        for i,key in enumerate(keys,1):
            row=exact_audit(key,durations=durations,centres=centres,U=U,n_buses=n_buses,sigma=args.noise_sigma,seeds=(seeds[0],),outer=8,inner=4,alpha=args.alpha,margin=args.margin,u_grid=u_grid,min_branch_share=.10,max_dominance=args.max_dominance,min_adaptive_advantage=args.min_adaptive_advantage,min_mean_branch_value=args.min_mean_branch_value,min_duration_branches=args.min_duration_branches,require_first_duration_differs=not args.allow_same_first_duration)
    ...
    selection_rule = (
        f"matched MOCU gates: adaptive mean>={args.min_adaptive_advantage:g} with positive LCB; "
        f"non-myopic LCB>0; branch mean>={args.min_mean_branch_value:g} with positive LCB; "
        f">={args.min_duration_branches} duration branches; dominance<={args.max_dominance:g}; "
        + ("first duration differs in all seeds" if not args.allow_same_first_duration
           else "first-duration difference not required")
    )
    summary={**run,"status":"pass" if passing else "no_duration_set_passed","n_global_candidates":len(candidates),"n_local_neighbors":len(local),"n_strict_finalists":len(final),"n_passing":len(passing),"search_is_exhaustive":False,"search_note":f"All 281 durations eligible; {args.global_samples:,} reproducible global samples, coordinate-local refinement, then strict three-seed MOCU validation.","selection_rule":selection_rule,"u_grid":u_grid.tolist(),"obs_indices":obs.tolist(),"elapsed_seconds":time.time()-started}
