...
def collect_campaign(campaign, horizons=(3,4,5), training_seed=101, evaluation_seed=1001, n_obs=0, noise_sigma=.005, methods=METHODS):
    """Validate full EIG runs and collect one comparison without rerunning methods."""
    methods=tuple(methods)
    if not methods or len(set(methods))!=len(methods):raise ValueError('Use distinct methods')
    root=Path(campaign).resolve()
    summaries={};provenance={};sensor_signature=None;system_name=None
    for t in horizons:
        run=root/f'T{t}'/'run'
        ...
        name=cfg['physical_config']['system']['name']
        if system_name is None:system_name=name
        assert name==system_name, 'Cannot pool different physical systems'
        ...
        assert len(rows)==done['records']==len(methods)*s['eval_systems']
        identities=None
        for m in methods:
    ...
    lines=[f'# {system_name.upper()} EIG — completed runs','',
           f'Train {training_seed}; eval {evaluation_seed}; N_obs={n_obs}; sigma={noise_sigma}. {len(methods)} requested methods evaluated.',
...
def main():
    ...
    a=p.parse_args()
    collect_campaign(a.campaign,tuple(map(int,a.horizons.split(','))),a.training_seed,a.evaluation_seed,a.n_obs,a.noise_sigma,tuple(a.methods.split(',')))
