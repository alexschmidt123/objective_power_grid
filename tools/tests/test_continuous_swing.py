...
class ContinuousSwingTests(unittest.TestCase):
    ...
    def test_matches_independent_12_second_integration(self):
        ...
        def rhs(t, y):
            k = min(int(t // 4), 2)
            return obs.sim._rhs(t-4*k, y, theta[0,:3], theta[0,3:],
                                Design(.05, 0, durations[k]))
        times = np.concatenate([obs.times+4*k for k in range(3)])
        reference = solve_ivp(rhs, (0.,12.), obs.initial_state(1)[0],
    ...
    def test_recording_window_and_continuous_duration(self):
        np.testing.assert_allclose(self.observer.times,
                                   [.0015625,1.0015625,2.0015625,3.,4.])
